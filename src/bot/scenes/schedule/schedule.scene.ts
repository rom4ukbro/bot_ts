...
import { choiceStudentText, choiceTeacherText, nextWeekText, todayText, previousWeekText, manualDateBtnEntry, changeQueryBtnText, allWeekBtnText, aboutBtnText, enterDateText, errorDateText, mainMenu } from '../../text';
...
writeDateScene.on('text', (ctx) => {
  try {
    ...
    ctx.session.weekShift = Math.round(
      (switchDay(
        moment.tz(ctx.message.text, 'DD.MM.YYYY', "Europe/Zaporozhye").format('dddd'),
        moment.tz(ctx.message.text, 'DD.MM.YYYY', "Europe/Zaporozhye").format('L'),
      ) -
        switchDay(moment.tz("Europe/Zaporozhye").format('dddd'), moment.tz("Europe/Zaporozhye").format('L'))) /
      1000 /
      60 /
      60 /
      24 /
    ...
    ctx.session.day =
      moment.tz(ctx.message.text, 'DD.MM.YYYY', "Europe/Zaporozhye").format('dd').charAt(0).toUpperCase() +
      moment.tz(ctx.message.text, 'DD.MM.YYYY', "Europe/Zaporozhye").format('dd').charAt(1);
