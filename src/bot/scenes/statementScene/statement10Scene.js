...
statement10Scene.on("text", (ctx) => {
  try {
    if (ctx.session.field == sessionDate) {
      ...
      if (
        !moment(absenceDate[0], "YYYY").isValid() ||
        !moment(absenceDate[1], "YYYY").isValid()
      ) {
        ctx.deleteMessage(ctx.message.message_id);
        return ctx.telegram
          .editMessageText(
      ...
      ctx.session.statementData.academicYears =
        absenceDate[0] + "/" + absenceDate[1];
...
statement10Scene.action(done, (ctx) => {
  try {
    ...
    const info = `Група: ${infoArr.group}\nПІБ: ${infoArr.pib}\nНомер: ${
      ...
      infoArr.academicYears
    }\nСесія: з ${infoArr.sDate} по ${infoArr.eDate}\nПричина: у зв\'язку з ${
      infoArr.reason
...
statement10Scene.action("no", (ctx) => {
  try {
    ctx.answerCbQuery("Можеш виправити те, що не правильно", {
      show_alert: true,
    });
    return ctx.editMessageText(
      statementEnter,
